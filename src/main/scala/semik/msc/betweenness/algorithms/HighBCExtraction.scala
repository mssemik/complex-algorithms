...
//  def outgoingEdges = graph.edges.groupBy(_.srcId).map(e => (e._1, Map(GraphPropTags.OutgoingEdges -> e._2.map(i => Edge(i.dstId)).toSeq)))
//
//  def incomingEdges = graph.edges.groupBy(_.dstId).map(e => (e._1, Map(GraphPropTags.IncomingEdges -> e._2.map(i => Edge(i.srcId)).toSeq)))
//
//  def vertices = {
//    graph.cache
...
//  GraphGenerators.logNormalGrapval h()

//  def repartition = partitioner.computePartitions(graph)
