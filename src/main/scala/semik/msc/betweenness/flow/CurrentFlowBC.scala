...
class CurrentFlowBC[VD, ED: ClassTag](graph: Graph[VD, ED], flowGenerator: FlowGenerator[CFBCVertex, Option[CFBCFlow]]) extends Serializable {
  ...
  def computeBC(phi: Int, epsilon: Double) = {
    ...
    val randomVertices = ctrw.sampleVertices(Math.ceil(flowGenerator.flowsPerVertex * 1.6) toInt)

    val initGraph = cfbcProcessor.initGraph.ops.joinVertices(randomVertices)((id, v, m) => {
      val sample = Random.shuffle(m.distinct.diff(List(id))).take(flowGenerator.flowsPerVertex)
...
//    println(s"NumOfVertices: ${getNumberOfFlows(g1)}, NumOfMsg: $msgCount")
    ...
    while (msgCount > 0) {
      ...
      if (i % 20 == 0) { g3.vertices.localCheckpoint(); g3.edges.localCheckpoint(); g3.vertices.count(); g3.edges.count() }
